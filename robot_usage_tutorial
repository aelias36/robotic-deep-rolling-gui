...
2. Test that everything is working by jogging the robot. To do this, on the pendant, click on menu > jogging. Pick up the pendant, press down the dead man switch, and then you should be able to move the joystick to jog the robot.

3. If workpiece offset needs to be restored, jog the roller to a centimeter above where the start point of the roll should be. Record the world x, y and z coordinates at this point (the first time that we did this we found that these coordinates were x: 0.4285 m, y:  -1.312 m, z:  1.0547 m). Type those values into workpiece offset.

4. Tare the force-torque sensor by clicking the tare button twice on the GUI.

5. You are now ready to begin the roll. Initiate the roll by pressing the dead man switch on the pendant, pressing play on the pendant, and then pressing start on the GUI.
